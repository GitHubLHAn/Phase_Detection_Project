...
void Handle_LoRa_RX(void)
{
  if(slave_status == BAT_LOW || slave_status == PLUG_CHARGER) return;
...
void Battery_Handle(Battery_Data_t *pBat)
{
  ...
  float vBat_now = (float)(pBat->adc_filterd/4095.0f)*3.3f*4.9f;  // tinh dien ap theo "pBat->adc_filterd"

  if(slave_status == NORMAL || slave_status == BAT_LOW)
  {
    if(vBat_now > (pBat->vBat_filtered + 0.1f)){
      slave_status = PLUG_CHARGER;
      mode_led = MODE_STS_R;
    }
  }

  if(slave_status == PLUG_CHARGER){
    if(pBat->vBat_filtered > (vBat_now + 0.1f)){
      slave_status = NORMAL;
      mode_led = MODE_OFF;
    }
  }

  pBat->vBat_filtered = vBat_now;     

  if(pBat->vBat_filtered < BATTERY_LOW && slave_status != PLUG_CHARGER)  
  {
  ...
  if(pBat->cnt_full_bat == 0 && pBat->vBat_filtered < BATTERY_FULL-0.1f && pBat->vBat_filtered > 3.0f)
  {
    pBat->cnt_full_bat++;
  }

  if(pBat->vBat_filtered > BATTERY_FULL && pBat->cnt_full_bat > 0){
