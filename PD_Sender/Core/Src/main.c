...
// bo tim3, set thu tu ngat, tim2->tim3->tim1
// tang ICFilter cua timer2 len 15
...
volatile uint16_t adc_raw_val = 0;   // Bi?n n�y s? T? �?NG c?p nh?t m?i 50us nh? DMA Circular
volatile uint16_t adc_ema_filtered = 0.0f;       // Gi� tr? sau khi l?c EMA
float alpha = 0.2f;
...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
volatile uint16_t cnt_filter = 0;
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
  if (htim->Instance == TIM1)
  {
    cnt_timetick++;
		...
		if(++cnt_filter >= 10){
			cnt_filter = 0;
			if (adc_ema_filtered == 0) {
        adc_ema_filtered = adc_raw_val;
			} else {
        adc_ema_filtered = (alpha*adc_raw_val) + ((1.0f - alpha) * adc_ema_filtered);
    }
		}
...
int main(void)
{
  ...
  HAL_TIM_Base_Start_IT(&htim1);
	...
	HAL_ADC_Start_DMA(&hadc1, (uint32_t*)&adc_raw_val, 1);
  ...
  while (1)
  {
		...
		if(cnt_timetick >= cycle)
    {
      cnt_timetick = 0;
		...
		if(flag_tx_log >= 1){
			flag_tx_log = 0;
			sprintf(uart_tx_log, "%u %u %u\n",++stt, adc_raw_val, adc_ema_filtered);
