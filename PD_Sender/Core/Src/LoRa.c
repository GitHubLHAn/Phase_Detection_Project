...
void Lora_Init(LoRa *pL, SPI_HandleTypeDef *phspi, uint16_t frequency){
	...
	pL->CS_pin                = NSS_Pin;
	pL->reset_port            = RF_RESET_GPIO_Port;
	pL->reset_pin             = RF_RESET_Pin;
...
\* ----------------------------------------------------------------------------- */
uint8_t LoRa_transmit(LoRa* _LoRa, uint8_t* data, uint8_t length, uint32_t timeout){
	uint8_t read;
